...
class CNN3DSlowFast(Module):
    def __init__(self, fc_type='original', retrain_depth=0, fc_depth=5, weights_fp=None, strict_weights=True, **kwargs):
        super(CNN3DSlowFast, self).__init__()
        self.fc_depth = fc_depth

        self.cnn = torch.hub.load('facebookresearch/pytorchvideo:main', 'slowfast_r50', pretrained=True)
        ...
        if fc_type == 'single':
            print(self.cnn)
            self.cnn.blocks[-1].proj = Linear(2304, 1)
